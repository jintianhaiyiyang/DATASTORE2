...
const ALIPAY_HOSTS = ["openapi.alipay.com", "openapi-sandbox.dl.alipaydev.com"];

export function getAlipayJumpUrl(value) {
  const url = new URL(value);
  if (url.protocol !== "https:" || !ALIPAY_HOSTS.includes(url.hostname) || url.username || url.password) {
    throw new Error("支付宝支付链接无效，请稍后重试");
  }
  return url.toString();
}
