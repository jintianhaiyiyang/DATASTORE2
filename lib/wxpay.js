...
export function unwrapWxResult(result) {
  ...
  if (result.error || result.errRaw || result.code || (Number.isFinite(status) && (status < 200 || status >= 300))) {
    let providerError = result.data || result;
...
// v2.2.x of the SDK replaces prepay_id with signed bridge parameters.
// Also accept a raw API v3 response for compatibility with other adapters.
export function getJsapiPayParams(data, wxpay, appId) {
  if (data.appId === appId && /^prepay_id=.+/.test(data.package || "") &&
      /^\d+$/.test(data.timeStamp || "") && data.nonceStr &&
      data.signType === "RSA" && data.paySign) {
  ...
  if (!data.prepay_id) throw new Error("JSAPI 预支付参数无效");
  const timeStamp = Math.floor(Date.now() / 1000).toString();
  const nonceStr = crypto.randomBytes(16).toString("hex");
  const pkg = `prepay_id=${data.prepay_id}`;
  return {
    appId, timeStamp, nonceStr, package: pkg, signType: "RSA",
