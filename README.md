...
- 全站公告栏：后台编辑内容、链接和样式，访客可关闭，内容变更后重新显示
...
- 完整注册功能需要 SMTP
- 完整购买功能需要微信支付 API v3（JSAPI 还需要公众号 OAuth）和/或支付宝开放平台应用
...
```dotenv
ALIPAY_APP_ID=2021...
ALIPAY_PRIVATE_KEY=应用私钥（PKCS1 或 PKCS8，可带或不带 PEM 头尾）
ALIPAY_PUBLIC_KEY=支付宝公钥（注意不是应用公钥）
ALIPAY_SELLER_ID=2088...（可选，填写后校验通知中的收款方）
ALIPAY_GATEWAY=（可选，沙箱为 https://openapi-sandbox.dl.alipaydev.com/gateway.do）
```

使用“公钥模式”加签。需要在支付宝开放平台为该应用签约 **电脑网站支付**（电脑端）和 **手机网站支付**（手机浏览器）。异步通知地址由下单接口自动传给支付宝，为：

```text
https://你的域名/api/notify/alipay
```

- 支付宝收银台在支付宝域名下打开，付款后跳回资源页并自动确认结果；未付款的收银台 30 分钟后失效。
- 微信内置浏览器会屏蔽支付宝页面，此时页面提示用户“在浏览器打开”后再付款。
- 通知使用支付宝公钥验签，并逐项核对本地订单号、金额、AppID（以及填写时的 PID）。查单使用 `alipay.trade.query` 并校验响应签名。

### 后台开关

后台“支付”页可分别开关微信支付和支付宝，并显示对应环境变量是否已配置。前台只显示“已开启且已配置”的方式；关闭的方式在服务端同样无法创建新订单，已发起的订单仍会正常确认。后台“公告”页可编辑全站公告栏。
...
5. 微信通知必须使用原始请求体通过平台证书验签；支付宝通知使用支付宝公钥验签。验签失败立即拒绝。
6. 通知或主动查单成功后，仍需逐项匹配本地订单号、支付渠道、金额、币种、AppID 和商户号；一个渠道的付款不能解锁另一个渠道的订单。
...
| `GET/PUT /api/site` | 读取公开，写入管理员 | 站点设置、公告和支付开关（PUT 只更新提交的字段） |
...
| `POST /api/notify/wechat` | 微信验签 | 微信异步支付通知 |
| `POST /api/notify/alipay` | 支付宝验签 | 支付宝异步支付通知 |
...
4. 在微信侧配置通知、支付目录和授权域名；在支付宝开放平台签约电脑网站支付/手机网站支付并上传应用公钥。
